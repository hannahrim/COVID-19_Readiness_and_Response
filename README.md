...
# Model (Data Analysis)
We will use a supervised machine learning classification model to classify health facilities and districts as either ready to provide health services safely (i.e. adequate readiness for infection control and prevention), or not ready.  We will use python with the sklearn, numpy, pandas, scipy modules to explore the accuracy, precision and recall of predictions from several approaches;  Logistic regression, Support Vector Models (SVM), Decision Trees, and Gradient Boosted Tree.  We wil explore the needs to scale our data appropriately, and use oversampling (SMOTE, SMOTEENN) where necessary.  We will vary predictors to find the optimum set which provides the highest predictive value.  
